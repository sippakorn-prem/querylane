...
interface Props {
  ...
  onConfirm: () => void
  onCancel: () => void
  variant?: "destructive" | "prod"
}

export function ConfirmDialog({ title, message, query, onConfirm, onCancel, variant = "destructive" }: Props) {
  useEffect(() => {
    ...
    >
      <div className="animate-in fade-in-0 zoom-in-95 duration-150 mx-4 w-full max-w-md rounded-xl border border-border bg-card shadow-2xl">
        ...
        <div className="flex items-start gap-3 border-b border-border px-5 py-4">
          <div className={`mt-0.5 flex size-8 shrink-0 items-center justify-center rounded-full ${isProd ? "bg-amber-500/15" : "bg-red-500/15"}`}>
            {isProd
          ...
          <div>
            {isProd && (
              <span className="mb-1 inline-flex items-center rounded px-1.5 py-0.5 text-[10px] font-semibold uppercase tracking-wide bg-amber-500/15 text-amber-400">
                Production
              </span>
            )}
